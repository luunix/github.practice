...
//{
//	int a, b=0;
//	for (a = 2; a <= 250; a += 2)	//for(①初始化，②判断，④调整）
//	{
//		b += a;		 //{③循环语句; }
//	}
//	printf("sum=%d", b);
...
//		printf("%d\t", a[i]);
//	}
//	for (i = 0; i < N - 1; i++) {              //外层循环 控制比较的轮数  
//		for (j = i + 1; j < N; j++) {        //内层循环 控制比较的次数 
//			if (a[i] > a[j]) {        //若 前面的元素逐一和后面的元素比较， 前面元素大于后面的元素 则交换 
//				temp = a[i];
//				a[i] = a[j];
//				a[j] = temp;
//			}
